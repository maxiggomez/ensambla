...
argument-hint: [feature o bug a resolver]
---

Usá la skill `mg-eng-loop` (`.agents/skills/mg-eng-loop/SKILL.md`) — fuente única del protocolo (DISCOVER → CAMINO → OpenSpec → PLAN DE TESTS con gate de aprobación → RED → GREEN → REFACTOR → VERIFY → REVIEW → cierre).
