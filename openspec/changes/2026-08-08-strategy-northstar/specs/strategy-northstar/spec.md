...
Delta de la capability `strategy-northstar` para el change `strategy-northstar`.
Extiende lo ya implementado en `strategy-okrs-core` (definición/lectura de la
North Star): agrega visión/misión/valores, input levers y pilares con el mapa
...
The system SHALL allow Dirección to define the Organization's vision, mission and
values, visible to the whole Organization.
...
The system SHALL allow creating strategic pillars that group one or more
Objectives, and SHALL show the strategic map as the cascade Vision → North Star →
Pillars → OKRs with the real progress of each Objective.
...
- THEN the cascade Vision → North Star → Pillars → OKRs is shown with each
...
able to read it. The system SHALL allow adding input levers each linkable to an
Objective.
