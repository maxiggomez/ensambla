...
- **UI Radar en español** (`/strategy-northstar`): vista de estrategia
...
- `app-shell` (navegación lateral) — change `2026-08-06-app-shell` pendiente.
  Esta página vive como ruta top-level `/strategy-northstar`, igual que
  `/culture-enps`, y se integra al shell cuando ese change exista.
...
- Código nuevo: `src/modules/strategy-northstar/{domain,application,infrastructure}`
  (extensiones) y `src/app/strategy-northstar/`.
