...
describe("mock dev users registry (1.1)", () => {
  it("exposes the preset users with dev ids, verified emails and full names", () => {
    ...
    expect(new Set(DEV_USERS.map((user) => user.email)).size).toBe(5);
  ...
  it("covers every role and provides two collaborators for minimum-N flows", () => {
    ...
    expect(roles).toEqual(["Colaborador", "Colaborador", "Direccion", "Direccion", "Lider"]);
  });

  it("keeps a dedicated Dirección identity without a seeded Organization", () => {
    expect(findMockUserById("dev_onboarding")).toMatchObject({
